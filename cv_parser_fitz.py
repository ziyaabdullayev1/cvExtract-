"""
CV Parser Module - PyMuPDF (Fitz) Optimized Version
Extracts structured data from CV/Resume PDFs into organized JSON format.
Uses PyMuPDF for faster PDF text extraction.
"""
...
class CVParserFitz:
    """Parse CV/Resume PDFs and extract structured information using PyMuPDF."""
    ...
    def __init__(self, pdf_path: str):
        ...
        self._duration_pattern = re.compile(r'\(([0-9]+\s*(?:year|month)s?[^)]*)\)')
        self._location_pattern1 = re.compile(r'[A-Z]{2,}[-,\s]+[A-Z]{2,}')
        self._location_pattern2 = re.compile(r'([A-Z][A-Za-z]+(?:[-\s][A-Z][A-Za-z]+)+)')
        self._location_pattern3 = re.compile(r'[A-Z][a-z]+[-,\s]+[A-Z][a-z]+')
        self._location_match_pattern = re.compile(r'^[A-ZİÖÜÇŞĞ][a-züğışçö]+[-,\s]+[A-Z][a-z]+$', re.IGNORECASE)
    
    def _extract_raw_text(self) -> str:
        """Extract all text from the PDF using PyMuPDF (faster than pdfplumber)."""
        if self.full_text:
    ...
    def extract_contact_info(self) -> Dict[str, str]:
        ...
        # Location - look for city-country pattern in first 20 lines
        for line in lines[:20]:
            line_stripped = line.strip()
            
            # Skip if line is just the name
            name = self.extract_name()
            if line_stripped == name or name in line_stripped and len(line_stripped) < len(name) + 5:
                continue
            
            # Pattern: "Istanbul, Turkey" (often mixed with other info using pipes)
            # First, try to extract from lines with pipes (header format)
            if '|' in line:
                # Split by pipes and look for location in each segment
                segments = [s.strip() for s in line.split('|')]
                for segment in segments:
                    # Look for "City, Country" pattern in this segment
            ...
            # If not found in piped format, look for standalone location line
            # Pattern: "City, Country" on its own line
            ...
            elif self._location_pattern1.search(line_stripped):
                location_match = self._location_pattern2.search(line_stripped)
                if location_match:
    ...
    def extract_languages(self) -> List[str]:
        ...
        for lang_match in re.finditer(r'LANGUAGES', text, re.IGNORECASE):
            ...
            # Check if this section contains programming languages or spoken languages
            # Spoken languages usually have proficiency levels (Native, Fluent, etc.)
            ...
            has_programming = any(prog in lang_text.lower() for prog in programming_langs[:5])  # Check first few
            
            # If it has proficiency levels OR spoken language names, and NOT programming keywords, it's spoken languages
            if (has_proficiency or has_spoken_lang) and not has_programming:
                # Split by common delimiters and clean
                langs = re.split(r'[,\n•·�]', lang_text)  # Added � for bullet that doesn't render properly
                cleaned_langs = []
                for lang in langs:
                    lang = lang.strip()
                    # Skip empty
                    ...
                    if lang.upper() in ['LANGUAGES', 'ADDITIONAL INFORMATION', 'PROGRAMMING LANGUAGES', 'CERTIFICATIONS']:
                        continue
                    # Skip if it starts with "For full list" or similar
                    if lang.lower().startswith(('for full', 'page ', 'www.', 'http')):
                        continue
                    # Clean up language entries (e.g., "English – Upper-Intermediate")
    ...
    def extract_certifications(self) -> List[str]:
        ...
        # Look for certifications section with improved patterns
        patterns = [
            r'Certifications?[:\s]*\n(.+?)(?=\nLANGUAGES|\nADDITIONAL|\nEDUCATION|\nEXPERIENCE|$)',
            r'CERTIFICATIONS[:\s]*\n(.+?)(?=\nLANGUAGES|\nADDITIONAL|\nEDUCATION|\nEXPERIENCE|$)',
        ]
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                # Filter out noise and section headers
                filtered_certs = []
                for cert in certs:
                    # Skip if it's a section header or noise
    ...
    def extract_education(self) -> List[Dict[str, str]]:
        ...
        # Look for education section with improved patterns
        patterns = [
            r'Education[:\s]*\n(.+?)(?=\nEXPERIENCE|\nCERTIFICATIONS|\nSKILLS|$)',
            r'EDUCATION[:\s]*\n(.+?)(?=\nEXPERIENCE|\nCERTIFICATIONS|\nSKILLS|$)',
        ]
    ...
    def extract_experience(self) -> List[Dict[str, Any]]:
        ...
        # First, try to find EXPERIENCE section explicitly
        ...
        if exp_section_match:
            ...
            for i, line in enumerate(exp_lines):
                # Look for position titles (usually come first after company or standalone)
                if re.search(r'(Specialist|Engineer|Developer|Manager|Analyst|Lead|Consultant|Intern|QA|Tester)', line, re.IGNORECASE):
                    if not any(word in line.lower() for word in ['testing', 'reported', 'performed', 'developed']):  # Skip responsibilities
                        if current_exp:
                            experiences.append(current_exp)
                        current_exp = {'position': line, 'company': '', 'period': '', 'location': '', 'responsibilities': []}
                
                # Look for company name (usually has location in it or comes after position)
                elif ',' in line and len(line) < 100:
                    # Likely "Company, Location" format
                ...
                elif line.startswith('•') or line.startswith('�'):  # � is sometimes how bullets appear
                    if current_exp:
        ...
        # Fallback: Known company keywords to help identify companies (be specific!)
        company_keywords = ['Tobacco', 'Cosmetics', 'Coca-Cola', 'IBM', 'Bottlers', 'Icecek', 'Avon', 
                          'British American', 'BAT', 'CCI', 'Turkey', 'Scorp']
        ...
        while i < len(lines):
            ...
            if is_company:
                ...
                while j < len(lines) and j < i + 15:
                    ...
                    date_match = self._date_pattern.search(next_line)
                    if date_match and not period:
                        period = date_match.group(1)
                        
                        # Check for duration in parentheses
                        duration_match = self._duration_pattern.search(next_line)
                        if duration_match:
                        ...
                        if j < len(lines):
                            ...
                            if self._location_match_pattern.match(loc_line):
                                location = loc_line
                                j += 1
                        break
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python cv_parser_fitz.py <cv_pdf_file> [output_json_file]")
        sys.exit(1)
